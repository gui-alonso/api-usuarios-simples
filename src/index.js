...
app.post("/api/users", async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      "INSERT INTO usuarios (name, email, passwordHash) VALUES (?, ?, ?)",
      [name, emailLowerCase, passwordHash]
    );

    const id = result.insertId;

    res.status(201).json({ id, name, email: emailLowerCase });
  } catch (err) {
...
app.get("/api/users/:id", async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      "SELECT id, name, email, createdAt FROM usuarios WHERE id = ?",
      [id]
    );
...
app.put("/api/users/:id", async (req, res) => {
  try {
    ...
    // Atualiza no banco
    await db.query(
      "UPDATE usuarios SET name = ?, email = ?, passwordHash = ? WHERE id = ?",
